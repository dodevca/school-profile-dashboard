                    ...
                    <div class="d-flex flex-column flex-md-row align-items-center justify-content-between order-md-2 w-100 w-md-auto">
                        ...
                        <div class="dropdown status-dropdown pr-md-3 mt-3 mt-md-0 border-right btn-new order-md-1">
                            <div class="dropdown-toggle" id="dropdownMenuButton03" data-toggle="dropdown">
                                <div class="btn bg-body"><span class="h6">Urutkan :</span> <?= ucwords($data['filter']) ?><i class="bi bi-chevron-down ml-2 mr-0"></i></div>
                            </div>
                            <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton03">
                                <a class="dropdown-item" href="<?= !empty($data['query']) ? base_url('pengumuman?query=' . $data['query'] . '&filter=terbaru') : base_url('pengumuman?filter=terbaru') ?>">Terbaru</a>
                                <a class="dropdown-item" href="<?= !empty($data['query']) ? base_url('pengumuman?query=' . $data['query'] . '&filter=terlama') : base_url('pengumuman?filter=terlama') ?>">Terlama</a>
                            </div>
                        </div>
                    </div>
                    <div class="pr-md-3 mb-3 mb-md-0 order-2 order-md-1 w-100 w-md-auto">
                        <a href="<?= base_url('pengumuman/tambah') ?>" class="btn btn-primary w-100 w-md-auto"><i class="bi bi-plus mr-1"></i>Tambah Baru</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card-transparent mb-0">
            <div class="card-header d-flex align-items-center justify-content-between p-0 pb-3">
                <div class="header-title">
                    <h4 class="card-title">Pengumuman Aktif</h4>
                </div>
            </div>
                                    ...
                                    <div class="card-body">
                                        <div class="d-flex flex-wrap align-items-center justify-content-between">
                                            <div class="d-flex align-items-center">
                                                <button type="button" class="btn p-0" data-toggle="modal" data-target="#modal<?= $result['id'] ?>">
                                                    ...
                                                    <div class="media align-items-center flex-wrap">
                                                        ...
                                                        <div class="text-muted mr-3"><i class="bi bi-eye mr-2"></i><?= $result['views'] ?></div>
                                                        <?php if($result['important'] == 1): ?>
                                                            <div class="bg-warning-light px-2 py-1 rounded">
                                                                <small>Penting</small>
                                                            </div>
                                                        <?php endif; ?>
                                                    </div>
                                            ...
                                            <div class="media align-items-center ml-auto mt-md-0 mt-3">
                                                <a href="<?= base_url('pengumuman/' . $result['id']) ?>" class="btn bg-primary-light mr-2"><i class="bi bi-pen"></i>Edit</a>
                                                <a href="<?= base_url('pengumuman/' . $result['id']) . '/delete' ?>" class="btn bg-secondary-light"><i class="bi bi-trash m-0"></i></a>
                                            </div>
                                        </div>  
                                    </div>
                                        ...
                                        <div class="modal-dialog modal-dialog-scrollable" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header justify-content-center">
                                                    <h5 class="modal-title" id="<?= $result['id'] ?>Title"><?= $result['title'] ?></h5>
                                                </div>
                                                <div class="modal-body">
                                                    <?= $result['content'] ?>
                                                    <?php if($result['attachment'] != null): ?>
                                                        <div class="text-center pt-3 border-top">
                                                            <a href="<?= base_url('uploads/pengumuman/' . $result['attachment']) ?>" class="btn btn-outline-secondary"><i class="bi bi-file-earmark mr-2"></i>Lampiran</a>
                                                        </div>
                                                    <?php endif; ?>
                                                </div>
                                                <div class="modal-footer border-0 justify-content-center">
                                                    <a href="<?= base_url('pengumuman/' . $result['id']) ?>" class="btn bg-primary-light"><i class="bi bi-pen"></i>Edit</a>
                                                    <button type="button" class="btn bg-secondary-light" data-dismiss="modal"><i class="bi bi-x"></i>Tutup</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                   ...
                   <?php else: ?>
                        <div class="col-12">
                            <div class="alert alert-warning w-100" role="alert">
                                <div class="iq-alert-text">Tidak ada pengumuman.</div>
                            </div>
                        </div>
                    <?php endif; ?>
                </div>
